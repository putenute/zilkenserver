...
my $queryString= 'SELECT * FROM digitemp';
my $query = $dbh->prepare($queryString);
$query->execute() or die $query_test->err_str;
while (my $res = $query->fetchrow_hashref() ) {
	#'SerialNumber' => '280C8687040000F5',
	#'Temp' => '18.75',
	#time' => '2013-12-08 12:46:15',
}



#my $sth = $dbh->prepare('SELECT * FROM digitemp');
#$results = $dbh->selectall_hashref('SELECT * FROM digitemp', 'id');
...
# by the right person.
system "cp /tmp/tempplot.gif /var/www/img/tempplot.gif";
system "chown www-data:www-data /var/www/img/tempplot.gif";
system "chmod 0644 /var/www/img/tempplot.gif";
